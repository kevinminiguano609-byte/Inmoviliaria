...
export function validateImageFiles(
  ...
  files.forEach((file, index) => {
    ...
    // Validar nombre (permitir espacios, paréntesis, tildes y guiones)
    const nameWithoutExt = file.name.replace(/\.[^/.]+$/, "");
    const safeName = /^[a-zA-Z0-9 .()áéíóúÁÉÍÓÚñÑ_-]+$/;
    if (!safeName.test(nameWithoutExt)) {
      // Sanitizar y continuar sin error
      const sanitized = sanitizeFilename(file.name);
      // No se agrega error, se asume que el nombre será corregido antes de la carga
    }
  });
