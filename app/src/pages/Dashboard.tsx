import { Building2, Users, CheckCircle, Eye } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const stats = [
    { icon: Building2, label: 'Total propiedades', value: properties.length, trend: '+12%', positive: true },
    { icon: Users, label: 'Leads nuevos', value: leads.filter(l => l.status === 'nuevo').length, trend: '+8%', positive: true },
    { icon: CheckCircle, label: 'Ventas del mes', value: 6, trend: '+20%', positive: true },
    { icon: Eye, label: 'Visitas web', value: '3,240', trend: '+15%', positive: true },
  ];
  ...
  return (
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {stats.map((s, i) => (
          <div key={i} className="bg-white rounded-xl p-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
            ...
            <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full ${s.positive ? 'bg-[#F0FFF0] text-[#4CAF50]' : 'bg-[#FFF5F5] text-[#F44336]'}`}>
      ...
      <div className="bg-white rounded-xl p-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)] mb-8">
        ...
        <div className="flex items-end gap-4 md:gap-8 h-[300px] px-4">
            ...
            <div key={i} className="flex-1 flex flex-col items-center gap-2">
              <div className="w-full flex flex-col items-center">
                ...
                <div
                  ...
                  style={{ height: `${(d.value / maxValue) * 220}px` }}
                />
              </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#F5F5F5]">
                {['Nombre', 'Email', 'Teléfono', 'Asunto', 'Fecha', 'Estado'].map(h => (
            ...
            <tbody>
              {leads.slice(0, 5).map(lead => (
                <tr key={lead.id} className="border-b border-[#E0E0E0] hover:bg-[#FAFAFA] transition-colors">
                  <td className="px-6 py-4 text-sm text-[#333333]">{lead.name}</td>
                  ...
                  <td className="px-6 py-4">
                    <span className="inline-block px-3 py-1 rounded-full text-xs font-medium"
                      style={{ backgroundColor: statusColors[lead.status]?.bg || '#F5F5F5', color: statusColors[lead.status]?.text || '#666666' }}>
